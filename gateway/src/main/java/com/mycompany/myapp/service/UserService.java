...
public class UserService {
    ...
    // Si vienen roles desde el frontend
    if (userDTO.getAuthorities() != null && !userDTO.getAuthorities().isEmpty()) {

        return Flux.fromIterable(userDTO.getAuthorities())
            .flatMap(authorityRepository::findById)
            .collectList()
            .flatMap(foundAuthorities -> {
                authorities.addAll(foundAuthorities);
                newUser.setAuthorities(authorities);
                return saveUser(newUser);
            })
            .doOnNext(user -> LOG.debug("Created User with custom roles: {}", user));
    }

    // Si NO vienen roles, asigna USER por defecto
    return authorityRepository
        .findById(AuthoritiesConstants.USER)
        .flatMap(defaultAuthority -> {
            authorities.add(defaultAuthority);
            newUser.setAuthorities(authorities);
            return saveUser(newUser);
        })
        .doOnNext(user -> LOG.debug("Created User with default ROLE_USER: {}", user));
});
    }
